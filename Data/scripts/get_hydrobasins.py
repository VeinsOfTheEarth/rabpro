...
file_positions = list(np.where(
    [file1['title'][0:7] == "hybas_1" for file1 in file_list])[0])
file_ids = [file_list[i]['id'] for i in file_positions] 
file_names = ["Data/HydroBasins/level_one/" + file_list[i]['title'] for i in file_positions]
[print(id, name) for id, name in zip(file_ids, file_names)]

[drive.CreateFile({'id':id}).GetContentFile(name) for id, name in zip(file_ids, file_names)]
...
hybas_12 = hybas_12.filterMetadata("HYBAS_ID", "greater_than", 7000000000).filterMetadata("HYBAS_ID", "less_than", 8000000000)

raw_export = batch.Export.table.toDrive(
  collection = hybas_12,
  description = "hybas_12",
  fileFormat = "SHP"
)
batch.Task.start(raw_export)
...
file_list = drive.ListFile({'q': "'root' in parents and trashed=false"}).GetList()
file_positions = list(np.where(
    [file1['title'][0:8] == "hybas_12" for file1 in file_list])[0])
file_ids = [file_list[i]['id'] for i in file_positions] 
file_names = ["Data/HydroBasins/level_twelve/" + file_list[i]['title'] for i in file_positions]
[print(id, name) for id, name in zip(file_ids, file_names)]
[drive.CreateFile({'id':id}).GetContentFile(name) for id, name in zip(file_ids, file_names)]
