...
# ---- format time column
test = data.copy()
test = test.rename(columns={"temperature_system:index": "date"}).head()
test = test.assign(
    year=lambda x: x.date.str.slice(0, 4),
    month=lambda x: x.date.str.slice(4, 6),
    day="01",
)
date = pd.to_datetime(test[["year", "month", "day"]])
test["date"] = date
test = test.drop(["year", "month", "day"], axis=1).filter(regex="^((?!system).)*$")

# ----

sns.lineplot(x="date", y="precip_mean", data=data)
