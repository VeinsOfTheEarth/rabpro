...
class profiler:
    ...
    """    
    def __init__(self, coords, da=None, name='unnamed', path_results=None, force_merit=False, verbose=True):
    ...
    def ensure_crs_match(self):
        ...
        if self.gdf.crs is not None:
            if self.EPSG != self.gdf.crs.to_epsg():
                raise ValueError('Provided EPSG is {}, but GeoDataFrame thinks EPSG is {}. Rectify before continuing.'.format(self.EPSG, self.coords.crs['init'][5:])) 
        else:
    ...
    def delineate_basins(self, search_radius=None, map_only=False):
        ...
        if self.method == 'hydrobasins':
            ...
            if search_radius is not None:
                ...
                ds_lonlat = np.array([self.gdf.geometry.values[-1].coords.xy[0][0], self.gdf.geometry.values[-1].coords.xy[1][0]])
                self.nrows, self.ncols = mu.nrows_and_cols_from_search_radius(ds_lonlat[0], ds_lonlat[1], search_radius, gdal.Open(dps['DEM_uda']).GetGeoTransform())
            else:
                self.nrows, self.ncols = 50, 50

            self.basins, self.mapped = sb.main_merit(self.gdf, self.da, nrows=self.nrows, ncols=self.ncols, map_only=map_only, verbose=self.verbose)
            
            # Ensure the provided coordinate was mappable
            if self.basins is None:
                if map_only is False:
                    print('Could not find a suitable flowline to map given coordinate and DA. No basin can be delineated.')
            else:
                ...
                if pct_diff > 10:
                    print('Check delineated basin. There is a difference of {} % between MERIT DA and polygon area.'.format(pct_diff))
    ...
    def basin_stats(self, years='all'):
        ...
        self.stats = ss.main(self.gdf, self.basins, years[0], years[1], verbose=self.verbose)
