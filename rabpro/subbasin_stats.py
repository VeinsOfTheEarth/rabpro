...
def main(cl_gdf, sb_inc_gdf, stYYYY, enYYYY, verbose=False):
    ...
    for r in range(len(control["rID"])):
        ...
        for ib, b in enumerate(control["bands"][r]):
            ...
            if verbose:
                print(
                    "Computing subbasins stats for {}, band {} of {}...".format(
                        rID, ib + 1, len(control["bands"][r])
                    )
                )
            ...
            rsmout, recycle = rsm.zonal_stats(
                sb_inc_gdf,
                control["rastpath"][r],
            ...
            statkeys = list(
                set(control["stats"][r] + ["count", "sum", "mean"])
            )  # counts and sums are always returned
            stats = dict()
            for sk in statkeys:
                if sk == "hyp":
                    stats["hypE"] = []
            ...
            try:
                del allvals, all_frac_areas, weights
            except:
                pass
            ...
            for ct in range(len(rsmout["rast"])):
                ...
                areagrid = rsmout["areagrid"][
                    ct
                ]  # area of each pixel (used for weighting statistics)
                clippedrast = rsmout["rast"][ct]  # raster array of data
                clippedrastmask = rsmout["mask"][
                    ct
                ]  # true where pixels are within the polygon
                frac_areas = rsmout["frac_area"][
                    ct
                ]  # fraction of each pixel that is within the polygon
                ...
                if len(valid_pix[0]) == 0:
                    ...
                    try:
                        ...
                        all_frac_areas = np.concatenate(
                            [all_frac_areas, frac_areas[valid_pix]]
                        )
                        weights = np.concatenate(
                            [weights, areagrid[valid_pix] * frac_areas[valid_pix]]
                        )
                        if "hypE" in stats:
                            allvals_area = np.concatenate(
                        ...
                        weights = np.array(
                            areagrid[valid_pix] * frac_areas[valid_pix], dtype=np.float
                        )
                        if "hypE" in stats:
                            allvals_area = areagrid[valid_pix]
                    ...
                    if (
                        ...
                        try:
                            ...
                            stats["std"].append(
                                np.sqrt(
                                    np.average(
                                        (allvals - np.average(allvals, weights=weights))
                                        ** 2,
                                        weights=weights,
                                    )
                                )
        ...
        if len(control["bands"][r]) > 1:
            ...
            for isk, sk in enumerate(stats.keys()):
                ...
                cols = np.arange(
                    startcol + isk, csv_df.shape[1] - isk, len(stats.keys())
                )
                for ts in timestats:
                    csv_fieldname = (
                        str(statct)
                    ...
                    if ts == "tavg":
                        if np.sum(pd.isna((csv_df.iloc[:, cols].values))) == len(cols):
                            ...
                            csv_df[csv_fieldname] = np.nanmean(
                                csv_df.iloc[:, cols], axis=1
                            )
                    elif ts == "tstd":
                        if np.sum(pd.isna((csv_df.iloc[:, cols].values))) == len(cols):
                            csv_df[csv_fieldname] = np.nan
                        else:
                            csv_df[csv_fieldname] = np.nanstd(
                                csv_df.iloc[:, cols], axis=1
                            )
                    elif ts == "trange":
                        if np.sum(pd.isna((csv_df.iloc[:, cols].values))) == len(cols):
                            ...
                            csv_df[csv_fieldname] = np.nanmax(
                                csv_df.iloc[:, cols], axis=1
                            ) - np.nanmin(csv_df.iloc[:, cols], axis=1)
...
def get_controls(DAmax, stYYYY, enYYYY):
    ...
    for i, row in rast_df.iterrows():
        ...
        if pd.isna(row["start:yyyy"]):
            ...
            control["bands"].append(
                [
                    int(x / row["dt (yr)"])
                    for x in np.arange(startband, startband + nbands, dtype=np.int)
                ]
            )
...
def hyps_curve(evals, areavals, nbins=20, rm_outliers=True):
    ...
    for b in range(len(breaks) - 1):
        if b == 0:
            area.append(
                np.sum(
                    areavals[np.logical_and(evals >= breaks[b], evals <= breaks[b + 1])]
                )
            )
        else:
            area.append(
